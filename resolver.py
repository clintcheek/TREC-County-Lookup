...
def resolve_one(lic: str, name: str, broker: str, api_key: str, cfg: dict[str, Any], cache: dict[str, Any], cache_path: Path, audit_path: Path) -> Result:
    ...
    # Use tiered acceptance rather than requiring every candidate to pass every hard gate.
    # Strong property-listing evidence remains a hard rejection, while weaker property
    # language is handled by the candidate score instead of discarding the result.
    minimum_identity = float(cfg["minimum_identity_score"])
    viable = [
        c for c in candidates
        if c.property_penalty < 0.85
        and (
            (c.source_tier >= 2 and c.identity_score >= minimum_identity)
            or (c.source_tier >= 4 and c.identity_score >= max(0.32, minimum_identity - 0.18))
            or (c.identity_score >= 0.78 and c.source_tier >= 1)
        )
    ]
    for c in viable[:15]:
    ...
    if not ranked:
        return Result(lic, name, broker, status="Unresolved", notes="No usable Texas business-address candidate could be geocoded. Candidate evidence was saved for review.", updated_at=now_utc())
