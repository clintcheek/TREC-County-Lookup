...
USER_AGENT = "TexasBrokerCountyResolver/2.0 (private license-location research)"
...
PROPERTY_PATHS = ("/property/", "/homedetail/", "/listing/", "/listings/", "/realestateandhomes-detail/", "/homes/")
...
def source_profile(url: str) -> tuple[int, str]:
    ...
    if host.endswith("trec.texas.gov") or host.endswith("texas.gov"):
        return 5, "Government"
    if host in {"har.com", "realtor.com", "loopnet.com", "crexi.com"}:
        return 3, "Professional directory"
...
def fetch_page(url: str, delay: float) -> tuple[str, str, list[str]]:
    try:
        time.sleep(delay)
        ...
        return title, text, list(dict.fromkeys(addresses))[:15]
    except Exception:
        return "", "", []
...
def resolve_one(lic: str, name: str, broker: str, api_key: str, cfg: dict[str, Any], cache: dict[str, Any], cache_path: Path, audit_path: Path) -> Result:
    ...
    queries = [
        f'"{name}" "{lic}"',
        f'"{name}" (contact OR office OR location) Texas',
        f'"{name}" "{broker}" address' if broker else f'"{name}" broker address Texas',
        f'site:har.com "{name}" broker',
    ]
    raw: list[Candidate] = []
    ...
    for q in queries:
        ...
        for row in rows:
            ...
            for address in extract_addresses(f"{title} {snippet}"):
                raw.append(build_candidate(lic, name, broker, address, url, title, snippet, "search snippet"))
            tier, _ = source_profile(url)
            preliminary_ident = identity_score(name, broker, lic, title, snippet, url)
            if url not in seen_pages and page_budget > 0 and tier >= 2 and preliminary_ident >= 0.45:
                seen_pages.add(url); page_budget -= 1
                ptitle, ptext, addresses = fetch_page(url, delay)
                for address in addresses:
                    raw.append(build_candidate(lic, name, broker, address, url, ptitle or title, ptext or snippet, "page"))
    ...
    viable = [c for c in candidates if not c.reject_reason and c.identity_score >= float(cfg["minimum_identity_score"]) and c.source_tier >= 2]
    for c in viable[:10]:
    ...
    for group in groups.values():
        ...
        final = min(0.99, best + consensus_bonus + (0.05 if authoritative >= 4 else 0))
        ranked.append((final, sorted(group, key=lambda c: c.total_score, reverse=True)))
    ranked.sort(key=lambda x: x[0], reverse=True)

    audit_candidates(audit_path, candidates[:25])
    if not ranked:
        return Result(lic, name, broker, status="Unresolved", notes="No candidate passed identity, source, property-page, and geocoding gates.", updated_at=now_utc())
    ...
    notes = f"Identity and address evaluated separately. {source_count} independent source domain(s) support the selected address. Property-listing candidates were rejected."
    return Result(
...
def write_output(input_file: Path, output_file: Path, checkpoint: dict[str, Result]) -> None:
    ...
    headers = ["Office Address", "City", "State", "ZIP", "County", "Resolution Status", "Confidence", "Identity Score", "Consensus Sources", "Evidence Type", "Evidence URL", "Secondary Evidence URL", "Resolution Notes", "Last Updated UTC"]
    ...
    widths = {"Office Address":38,"City":18,"State":9,"ZIP":12,"County":18,"Resolution Status":18,"Confidence":12,"Identity Score":14,"Consensus Sources":18,"Evidence Type":26,"Evidence URL":52,"Secondary Evidence URL":52,"Resolution Notes":55,"Last Updated UTC":22}
    for h, width in widths.items(): ws.column_dimensions[get_column_letter(existing[h])].width = width
    ws.freeze_panes = "A2"; ws.auto_filter.ref = ws.dimensions
    output_file.parent.mkdir(parents=True, exist_ok=True); wb.save(output_file)


def should_process(old: Result | None, mode: str) -> bool:
    if old is None: return True
    if mode == "recheck_all": return True
    if mode == "recheck_review": return old.status in {"Needs Review", "Unresolved"}
    return False
...
def main() -> int:
    ...
    ap.add_argument("--mode", choices=["new", "recheck_review", "recheck_all"], default="new")
    args = ap.parse_args()
    cfg = json.loads(Path(args.config).read_text(encoding="utf-8"))
    api_key = os.environ.get("SERPER_API_KEY", "").strip()
    if not api_key: raise SystemExit("SERPER_API_KEY is missing.")
    input_file, output_file = Path(cfg["input_file"]), Path(cfg["output_file"])
    checkpoint_path, audit_path, cache_path = Path(cfg["checkpoint_file"]), Path(cfg["candidate_audit_file"]), Path(cfg["search_cache_file"])
    checkpoint, cache = load_checkpoint(checkpoint_path), read_search_cache(cache_path)
    wb = load_workbook(input_file, read_only=True, data_only=True); ws = wb.active
    _, lic_col, name_col, broker_col = find_columns(ws)
    pending: list[tuple[str,str,str]] = []
    limit = args.max_rows or int(cfg["max_rows_per_run"])
    for row in ws.iter_rows(min_row=2, values_only=True):
        lic, name = canonical_license(row[lic_col-1]), clean(row[name_col-1])
        broker = clean(row[broker_col-1]) if broker_col else ""
        if lic and name and should_process(checkpoint.get(lic), args.mode):
            pending.append((lic, name, broker))
            if len(pending) >= limit: break
    wb.close()
    print(f"Checkpoint={len(checkpoint)}; mode={args.mode}; processing={len(pending)}")
    with ThreadPoolExecutor(max_workers=int(cfg["workers"])) as pool:
        futures = {pool.submit(resolve_one, lic, name, broker, api_key, cfg, cache, cache_path, audit_path): lic for lic,name,broker in pending}
        for n, fut in enumerate(as_completed(futures), 1):
            try: result = fut.result()
            except Exception as exc:
                lic = futures[fut]; result = Result(lic, "", status="Error", notes=f"{type(exc).__name__}: {exc}", updated_at=now_utc())
            checkpoint[result.license_number] = result
            if n % 10 == 0 or n == len(futures):
                save_checkpoint(checkpoint_path, checkpoint); save_search_cache(cache_path, cache)
                print(f"Checkpoint {n}/{len(futures)}")
    save_checkpoint(checkpoint_path, checkpoint); save_search_cache(cache_path, cache); write_output(input_file, output_file, checkpoint)
    counts = defaultdict(int)
    for r in checkpoint.values(): counts[r.status] += 1
